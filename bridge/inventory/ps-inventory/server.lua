...
RegisterNetEvent('zero_utils:server:qb-inventory:OpenInventory'..zutils.name, function(inv_type, id)
    ...
    if inv_type == 'stash' and stashes[id] then
        ...
        ps_inventory:OpenInventory(src, id, {
            label = stash.label,
            slots = stash.slots,
...
function inventory.getInv(inv)
    local inven = ps_inventory:GetInventory(inv)
...
function inventory.getAvailableWeight(inv)
    if tonumber(inv) then
        return ps_inventory:GetFreeWeight(inv)
    end

    local check_inv = ps_inventory:GetInventory(inv)
    if check_inv then
        return check_inv.maxWeight - ps_inventory:GetTotalWeight(inv)
    else
        return 0
    end
end
...
function inventory.getAvailableSlots(inv)
    local _, abailable_slots = ps_inventory:GetSlots(inv)
    return abailable_slots
end

function inventory.canCarryItem(src, item, count, metadata)
    if metadata then
        printwarn("Metadata search is not supported in ps-inventory bridge at this time")
    end
...
function inventory.addItem(inv, item, count, metadata, slot, cb)
    if cb then printwarn("Callback is not supported in ps-inventory bridge") end

    local result = ps_inventory:AddItem(inv, item, count, slot, metadata or false, zutils.name .. " add item")
    ...
    if tonumber(inv) then
        TriggerClientEvent('ps-inventory:client:ItemBox', inv, QBCore.Shared.Items[item], "add", count)
        Wait(100)
    end
...
function inventory.removeItem(inv, item, count, metadata, slot)
    ...
    local result = ps_inventory:RemoveItem(inv, item, count, slot, zutils.name .. " remove item")
    ...
    if tonumber(inv) then
        TriggerClientEvent('ps-inventory:client:ItemBox', inv, QBCore.Shared.Items[item], "remove", count)
    end
...
function inventory.hasItem(inv, item, count, metadata)
    ...
    if tonumber(inv) then
        local result = ps_inventory:HasItem(inv, item, count)
        if result then
            return true
        end
